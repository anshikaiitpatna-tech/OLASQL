...
create view ride_dis_avg as
 select Vehicle_Type, AVG(Ride_Distance) as avg_dis
from bookings
group by 
 Vehicle_Type;

 select * from ride_dis_avg;
...
CREATE VIEW revenue_by_vehicle AS
SELECT 
    Vehicle_Type, 
...
-- Customers ke cancel karne ka major reason:
CREATE VIEW customer_cancel_reasons AS
SELECT Canceled_Rides_by_Customer, COUNT(*) AS total_count
FROM bookings
WHERE Booking_Status = 'Canceled by Customer'
GROUP BY Canceled_Rides_by_Customer;

-- Drivers ke cancel karne ka major reason:
CREATE VIEW driver_cancel_reasons AS
SELECT Canceled_Rides_by_Driver, COUNT(*) AS total_count
FROM bookings
WHERE Booking_Status = 'Canceled by Driver'
GROUP BY Canceled_Rides_by_Driver;

CREATE VIEW top_5_busy_routes AS
SELECT 
    Pickup_Location, 
    Drop_Location, 
...
CREATE VIEW avg_turnaround_time AS
SELECT 
    Vehicle_Type,
...
CREATE VIEW cancellation_revenue_impact AS
SELECT 
    Booking_Status,
    COUNT(*) AS ride_count,
...
CREATE VIEW cancellation_by_hour AS
SELECT 
...
CREATE VIEW rating_vs_cancellation AS
SELECT 
    CASE 
...
FROM bookings
WHERE Booking_Status = 'Success' AND Payment_Method != 'null'
GROUP BY Payment_Method;

CREATE VIEW customer_ride_frequency AS
SELECT 
    Customer_ID,
    COUNT(*) AS total_rides,
